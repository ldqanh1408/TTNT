...
# Thuật toán cố định: Tournament + Elitism  |  Uniform Crossover  |  Gaussian Mutation
...
#python ga/train_ga.py --gen 100 --pop 50
class GenomeIndividual:
    __slots__ = ("network", "fitness", "age", "id")
    _counter = 0
    ...
    def __repr__(self) -> str:
        return f"<GenomeIndividual #{self.id} fitness={self.fitness:.2f} age={self.age}>"
...
class GeneticAlgorithm:
    ...
    def mutate(self, individual: GenomeIndividual) -> None:
        weights = individual.network.get_flat_weights()
        size    = weights.size
        mask    = np.random.rand(size) < self.ga_cfg.MUTATION_RATE
        noise   = np.random.randn(size) * self.ga_cfg.MUTATION_STRENGTH
    ...
    def load_best(cls, path: str) -> tuple[DinoNet, dict]:
        ...
        meta = {"fitness": data["fitness"], "generation": data["generation"]}
        print(f"  [GA] Đã tải best dino từ {path}  "
              f"(fitness={meta['fitness']:.2f}, gen={meta['generation']})")
        return net, meta
